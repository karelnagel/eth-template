...
function encodeAnchors() {
    const buffers = new Array(DEFAULT_TRUST_ANCHORS.length);
    for (let i = 0; i < buffers.length; i++) {
        buffers[i] = packet.answer.encode(DEFAULT_TRUST_ANCHORS[i]);
    }
    return Buffer.concat(buffers);
}
function decodeProofs(data) {
    const proofs = [];
...
beforeEach(async(), {
    ...
    for(let = id, of = Object.keys(algorithms)) {
        await;
        oracleContract.setAlgorithm(id, algorithms[id].address);
    },
    for(let = id, of = Object.keys(digests)) {
        await;
        oracleContract.setDigest(id, digests[id].address);
    }
});
